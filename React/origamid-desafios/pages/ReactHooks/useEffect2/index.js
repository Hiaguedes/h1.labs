...
export default function useStateAnswer() {
  ...
  useEffect(async() => {
    if(localStorage.getItem('produto')) {
      await fetchApi(localStorage.getItem('produto'))
    } 

  }, [])

  useEffect(() => {
    produto.id.length >0 && localStorage.setItem('produto', produto.id);
  }, [produto])
  ...
  return (
    <>
    <Head>
      <title>Exercício de useEffect</title>
    </Head>
    <Layout>
      <pre>Resolução do exercício de useEffect da Origamid</pre>
    ...
    {produto.id.length>0 && <Produto produto={produto} />}
